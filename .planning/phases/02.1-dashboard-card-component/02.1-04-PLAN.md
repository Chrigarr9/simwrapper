...
must_haves:
  truths:
    ...
    - "Each card has exactly one enlarge button (in DashboardCard header)"
    - "Content components have no frame/header/button code (render only visualization)"
  artifacts:
    ...
    - path: "src/plugins/interactive-dashboard/components/cards/DataTableCard.vue"
      provides: "Table content only, no frame/buttons"
  key_links:
    ...
    - from: "SubDashboard.vue"
      to: "InteractiveDashboard.vue"
      via: "embedded: true prop"
      pattern: "embedded.*true"
---
...
# SubDashboard embeds InteractiveDashboard with embedded: true
...
<task type="auto">
  <name>Task 4: Verify sub-dashboard card behavior</name>
  <files>src/plugins/interactive-dashboard/components/cards/SubDashboard.vue</files>
  <action>
Verify that sub-dashboards use the same DashboardCard architecture automatically.

**Per the guidance:** SubDashboard.vue embeds InteractiveDashboard with `embedded: true`, so if InteractiveDashboard is fixed, sub-dashboards should work automatically.

**Verification only (no code changes expected):**
1. Confirm SubDashboard passes `embedded: true` to InteractiveDashboard
2. Confirm InteractiveDashboard handles embedded prop (if any special handling exists)
3. Test that cards in sub-dashboards get DashboardCard wrapper

**If issues found:**
- Check if InteractiveDashboard needs different behavior when embedded
- Ensure fullscreen from sub-dashboard expands to full viewport (not just sub-dashboard container)
- This may already be handled by the position: fixed styling in DashboardCard

**Expected behavior:**
- Cards in sub-dashboards have identical frame/header/buttons as main dashboard cards
- Fullscreen on sub-dashboard card expands to full viewport
- All DashboardCard features work in sub-dashboard context
  </action>
  <verify>
1. Open dashboard with sub-dashboards
2. Click a row to show sub-dashboard
3. Verify sub-dashboard cards have frame with enlarge button
4. Click enlarge on sub-dashboard card
5. Card expands to full viewport (not just sub-dashboard area)
6. Press Escape - card restores correctly
  </verify>
  <done>Sub-dashboard cards use DashboardCard and behave identically to main dashboard cards</done>
</task>
...
5. Sub-dashboards automatically inherit the new architecture
  </what-built>
  <how-to-verify>
