        ...
        linkable-card-wrapper(v-if="getCardComponent(card) && filterManager && linkageManager"
          ...
          template(v-slot="{ filteredData, baselineData, showComparison, hoveredIds, selectedIds, handleFilter, handleHover, handleSelect }")
            component.dash-card(v-if="card.visible"
              ...
              :y-columns="card.yColumns"
              :barmode="card.barmode"
              :color-by-column="card.colorByColumn"
              :y-axis-title="card.yAxisTitle"
              :x-axis-title="card.xAxisTitle"
              :annotations="card.annotations"
              :reference-lines="card.referenceLines"
              :source-column="card.sourceColumn"
              :target-column="card.targetColumn"
              :value-column="card.valueColumn"
              :node-color-map="card.nodeColorMap"
